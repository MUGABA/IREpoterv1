...
def create_new_user():
	...
	if email == '' or username == '' or password == '':
		return jsonify({"message" : "email, username and password cannot be empty"}),400
	...
	userlist.add_a_user(new_user)
	return jsonify({"user" : userlist.user_list[-1]}),201


def getall_users():
	return jsonify({"users" : userlist.get_all_users()}),200
...
def sign_in_user():
	...
	return jsonify({"message" : "The user doesnot exist"}),401
